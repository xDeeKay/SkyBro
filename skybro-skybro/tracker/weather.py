...
Fetches from Open-Meteo (no API key) and computes moon phase locally.
Stores results in SQLite for the dashboard to read.
"""

import math, time, json, sqlite3, logging, requests
...
def process_weather(data, conn):
    ...
    for i in range(start_i, min(start_i + 25, len(times))):
        hcloud  = (hourly.get("cloud_cover") or [])[i] if i < len(hourly.get("cloud_cover",[])) else 0
        hvis    = (hourly.get("visibility")  or [])[i] if i < len(hourly.get("visibility",[])) else 10000
        hprecip = (hourly.get("precipitation") or [])[i] if i < len(hourly.get("precipitation",[])) else 0
        hwind   = (hourly.get("wind_speed_10m") or [])[i] if i < len(hourly.get("wind_speed_10m",[])) else 0
        h_astro = astronomy_score(hcloud or 0, hvis or 10000, hprecip or 0, hwind or 0)
        hourly_list.append({
            "time":         times[i],
            "temp":         (hourly.get("temperature_2m") or [])[i],
            "precip_prob":  (hourly.get("precipitation_probability") or [])[i],
            "precipitation":(hourly.get("precipitation") or [])[i],
            "weather_code": (hourly.get("weather_code") or [])[i],
            "description":  wmo_desc((hourly.get("weather_code") or [])[i]),
            "wind_kts":     round(hwind, 1),
            "cloud_cover":  hcloud,
            "uv_index":     (hourly.get("uv_index") or [])[i],
    ...
    for i in range(len(dtimes)):
        dcloud  = 0  # daily doesn't include cloud avg, use code as proxy
        d_code  = (daily.get("weather_code") or [])[i]
        d_precip= (daily.get("precipitation_sum") or [])[i] or 0
        d_wind  = (daily.get("wind_speed_10m_max") or [])[i] or 0
        ...
        daily_list.append({
            ...
            "precip_sum":   d_precip,
            "precip_prob":  (daily.get("precipitation_probability_max") or [])[i],
            "wind_max_kts": round(d_wind, 1),
            "uv_max":       (daily.get("uv_index_max") or [])[i],
...
def moon_phase(dt=None):
    """
    Calculate moon phase without external libraries.
    Returns phase name, illumination %, age in days, and next full/new moon.
    ...
    age   = days % LUNAR_CYCLE           # days since last new moon
    frac  = age / LUNAR_CYCLE            # 0.0 → 1.0
    ...
    # Next full and new moon timestamps
